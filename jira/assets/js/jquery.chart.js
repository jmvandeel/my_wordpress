// http://www.jqueryscript.net/chart-graph/jQuery-Plugin-To-Convert-Tabular-Data-Into-Donut-Charts-Chart-js.html
...
(function($){
	$.fn.donutChart = function(options) {
		...
		function drawDonutChart(context, graph, items){
			...
			drawGraphShadow(context, graph, centerX + 5, centerY + 5, radius);
			...
			for(var idx in items){
				...
				if(graph.legendBox.isLeft){
					...
					drawLegendEntry(context, item, centerX - (graph.legendSize*2), graph.y + graph.height - graph.legendSize*2 + (graph.legendSize * 1.1 * idx), graph.legendSize, sum);
				}
		...
		function drawGraphShadow(context, graph, x, y, radius, color){
			context.beginPath();
			if(graph.hasShadow){
		...
		function createDonutChart(table){
			...
			graph.hasShadow = opts.hasShadow;
